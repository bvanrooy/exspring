...
public class MainController {
	...
	public String submitLogin(Person personLogin, BindingResult bindingResult) {
		...
		if (personLoggedIn != null) {
			...
			bindingResult.reject("incorrectLogin", "Email or password incorrect");
			return PageMapping.LOGIN.getPageMappingName();
		}
	...
	public String submitPersonForm(@Valid Person person, BindingResult bindingResult, Model model) {
		...
		try {
			...
			bindingResult.reject("There was a problem saving the person");
		} catch (PersonAllreadyExistsException e) {
			logger.info("POST : personform PersonAllreadyExistsException catched");
			bindingResult.reject("personexits", "Person allready exists");
		}
	...
	public String submitDeletePerson(Person personToDelete, BindingResult bindingResult, Model model) {
		...
		if (personService.personCanBeDeleted(personToDelete, personLoggedIn)) {
			try {
				personService.deletePerson(personToDelete.getPersonId());
				model.addAttribute("msg", "Person successfully deleted");
			} catch (PersonNotFoundException e) {
				bindingResult.reject("personnotfound", "Person not found");
			}

		}
		else {
			bindingResult.reject("youcannotdeleteyourown","You cannot delete yourself");
		}
		return PageMapping.DELETEPERSON.getPageMappingName();
